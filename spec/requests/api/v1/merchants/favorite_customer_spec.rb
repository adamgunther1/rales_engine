...
RSpec.describe "Merchants/FavoriteCustomer API" do
...
# binding.pry
      ...
      expect(raw_customer).to have_key("total")
      ...
      expect(raw_customer["total"]).to be_a Float
      expect(raw_customer["id"]).to eq(invoice2.customer.id)
      expect(raw_customer["first_name"]).to eq(invoice2.customer.first_name)
      expect(raw_customer["last_name"]).to eq(invoice2.customer.last_name)
      expect(raw_customer["total"]).to eq(24712.95)
    end
  end
end